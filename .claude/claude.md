...
- [x] 단원 등록 — 초1~고3 교육과정 단원 121개 시드 (`scripts/seed-units.ts`, 목록은 `scripts/unit-data.ts`)
- [ ] 실제 학생 계정 생성
...
### 2026-09-22 (단원 시드 · 한글 사용자명 버그 수정)

- 초1~고3 교육과정 단원 121개 시드. 1·2학기를 합치고 학기별 중복 단원명은 통합.
  고3은 선택과목이라 `[확통]`·`[미적]`·`[기하]` 접두사로 구분.
  풀이 과정 필수 여부는 초1~초2 false, 초3 이상 true로 기본 설정 (관리자 화면에서 조정 가능)
- **한글 사용자명으로 계정 생성이 실패하던 버그 수정**
  - 원인: 사용자명 → Auth 이메일 변환 시 한글이 전부 제거돼 유효하지 않은 주소가 됨.
    단순 치환만 하면 '곽민준'·'곽민서'가 모두 '___'가 되어 충돌하기도 함
  - 수정: 이메일에 못 쓰는 문자가 있으면 원본 이름의 sha256 앞 16자를 덧붙여 고유 주소 생성.
    영문·숫자 사용자명(admin 등)은 기존 주소가 그대로 유지되어 로그인에 영향 없음
- 소프트 삭제된 계정과 같은 이름을 쓸 때 "이미 사용 중" 대신 이유를 밝히도록 메시지 수정
- Auth 계정 생성 실패 시 Supabase 원인 메시지를 함께 반환하도록 수정
- Vercel 환경변수에 GEMINI_MODEL 추가 (모델 단종 시 코드 수정 없이 교체)

### 2026-09-23 (테스트 피드백 반영: 다음 문제 · 그림 출제 · 취약 유형 보강)

- **"같은 유형 다음 문제" 버튼** — 문제를 끝내면 문제은행으로 돌아가지 않고
  방금 푼 문제와 같은 조건(학년·단원·난이도)으로 바로 다음 문제를 출제.
  `GET /api/problems/[id]`가 출제 조건과 표시용 이름을 함께 내려주도록 확장.
  내 문제 풀기(user_uploaded)는 조건이 없어 기존처럼 문제은행으로 이동
- **문제 그림(SVG) 출제** — 도형·그래프·수직선처럼 그림이 필요한 문제는
  Gemini가 `figure_svg`를 함께 생성 (`problems.figure_svg`에 고정 저장, 마이그레이션 003).
  말로 된 설명 대신 실제 도형에 수치를 표기하게 프롬프트 지시.
  AI가 만든 마크업이므로 `sanitizeSvg`로 script/이벤트/외부링크를 걷어내고 저장·렌더.
  채점·포기 풀이 프롬프트에도 SVG 소스를 함께 보내 그림 속 수치를 읽게 함
- **취약 유형 보강 출제** — 유형별 "한 번에 맞힌 비율"이 80% 미만이면(완료 3문제 이상)
  최근 틀린 문제와 반복된 지적 사항을 프롬프트에 넣어 비슷한 유형으로 출제.
  `GET /api/units`가 학생에게는 단원별 정답률·보강 대상 여부를 함께 내려주고,
  문제은행 단원 선택과 문제풀이 화면에 안내 문구 표시
- **LaTeX 노출 수정** — AI 피드백·문제 본문에 "$4\\text{cm}$"처럼 LaTeX가 그대로 보이던 문제.
  프롬프트에 "LaTeX·마크다운 금지, 유니코드 기호로만" 규칙(`PLAIN_MATH_RULE`)을 넣고,
  이미 저장된 기록까지 커버하도록 화면에 그릴 때 `toReadableMath`로 한 번 더 정리
  (4cm, 1/2, x², √16, π, 30°, ×, ÷). 문제풀이·공부기록·풀다 만 문제 목록에 모두 적용

### 2026-09-23 (출제 지연 문제)

- "문제은행 출제가 안 된다" → 실제로는 **실패가 아니라 27~52초 지연**.
  서버 로그·DB 확인 결과 해당 시각의 요청 2건 모두 문제 생성·저장까지 완료돼 있었음.
  그림(SVG)을 함께 생성하면서 응답이 길어진 것이 원인
- 속도: Gemini 호출을 JSON 응답 모드(`responseMimeType: application/json`)로 고정하고
  SVG 크기를 제한(1000자·요소 15개) → 로컬 기준 18s→12s, 16s→13s, 9s→7s
- AI 라우트 4곳에 `export const maxDuration = 60` 추가 (기본 10초 제한 회피)
- 화면: 경과 초 표시 + 소요 시간 안내. `postJson`(lib/api/request.ts)으로 타임아웃(75초)과
  JSON이 아닌 응답(504 HTML 등)을 사람이 읽을 수 있는 오류로 변환
- 추가 최적화: Gemini의 내부 추론(thinking)이 지연의 주원인 (초5 출제 13.5초 중 thought 1549토큰).
  끄면 4배 빨라지지만 **어려운 문제에서 정답이 틀린다** (고1·고2·중3 난이도 상 4문제 중 3문제 오답 —
  저장된 정답이 곧 채점 기준이라 치명적). 그래서 초등 + 난이도 하/중에서만 추론을 끈다
  (`shouldSkipThinking`). 측정: 초등 하/중 2.4~4.4초, 초등 상 11초, 중등 14초, 고등 14~19초
- JSON 응답이 드물게 깨져 "AI 응답 형식이 올바르지 않습니다"로 노출되던 문제 →
  `generateJson`으로 1회 재시도

### 2026-09-23 (출제 후 "문제를 찾을 수 없습니다")

- 증상: 문제은행에서 출제하면 문제는 만들어지는데 풀이 화면에서 문제를 못 찾음
- 원인: Postgres는 `CREATE VIEW ... SELECT *` 시점의 컬럼 목록을 고정한다.
  003에서 `problems.figure_svg`를 추가했지만 `active_problems` 뷰를 재생성하지 않아
  뷰로 figure_svg를 조회하면 오류 → 조회 결과가 null → 404.
  (001에서 content를 추가할 때는 뷰를 다시 만들었는데 003에서 빠뜨렸다)
- 조치: 마이그레이션 004로 뷰 재생성 + 문제 조회 3곳을 기본 테이블 + 동일 조건으로 변경해
  뷰 컬럼 드리프트에 영향받지 않게 함 + `npm run check-db`에 같은 드리프트 점검 추가
